...
@export_range(0, 500, 1) var clampzone_size : float = 150
...
func _update_joystick(touch_position: Vector2) -> void:
	...
	if use_input_actions:
		...
		#	dir_x = output.x
		#	pass
		#	#_update_input_x(action_right,output.x)
		#	#_update_input_action(action_right, output.x)
		#elif output.x < 3.0:
		#	dir_x = output.x
		#	pass
		#else :
		if output.x < 0.3 and output.x > -0.3: 
			dir_x = 0.0
		else:
			dir_x = output.x
			#_update_input_x(action_left,-output.x)
			#_update_input_action(action_left, -output.x)
		if output.y < 0.3 and output.y > -0.3: 
			dir_y = 0.0
		else:
			dir_y = output.y
		#if output.y > 0:
		#	_update_input_d(action_down,output.y)
		#	#_update_input_action(action_down, output.y)
		#else:
			_update_input_x(action_up,-output.y)
			#_update_input_button(action_up,-output.y)
...
func _update_input_j(action:String, value:float):
	if player.player_state == player.WATER_STATE:
		if value > 0.3:
			Input.action_press(action, value)
		elif Input.is_action_pressed(action):
			Input.action_release(action)
		...
		if value > 0.3:
			...
			Input.action_release(action)
func _update_input_d(action:String, value:float)->void:
	if player.player_state == player.WATER_STATE:
		if value > 0.3:
			Input.action_press(action, value)
		elif Input.is_action_pressed(action):
		...
		if value > 0.7 and !down_pressed:
			Input.action_press(action)
			down_pressed= true
		else:
			down_pressed = false
func _update_input_x(action : String,value:float)->void:
	if value > 0.3:
		Input.action_press(action, value)
	elif Input.is_action_pressed(action):
		Input.action_release(action)
